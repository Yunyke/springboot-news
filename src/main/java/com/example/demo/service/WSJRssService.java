...
public class WSJRssService {
    ...
    public List<Map<String, String>> getReutersNews() {
        ...
        try {
            ...
            for (int i = 0; i < items.getLength(); i++) {
             ...
             // 先抓 media:content 的圖片
             NodeList mediaContentList = item.getElementsByTagName("media:content");
             if (mediaContentList.getLength() > 0) {
                 Element media = (Element) mediaContentList.item(0);
                 if (media.hasAttribute("url")) {
                     imageUrl = media.getAttribute("url");
                 }
             }

             // 如果 media:content 沒抓到，再試 enclosure
             if (imageUrl.isEmpty()) {
